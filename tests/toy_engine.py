...
def write_b_surface_output_file(output_file_name, k_on_src, b_transition_probs,
    ...
    for key in b_transition_probs:
        if key == "escaped":
            escaped = int(round(b_transition_probs["escaped"] * N_TRAJS))
        else:
            key1 = key
            name1 = "b_%s" % key
            n1 = int(round(b_transition_probs[key] * N_TRAJS))
            
    for key in milestone_transition_probs:
        if key == "escaped":
            escaped2 = int(round(milestone_transition_probs["escaped"] * n1))
            pass
        else:
            name2 = "%s_%s" % (key1, key)
            n2 = int(round(milestone_transition_probs[key] * n1))
